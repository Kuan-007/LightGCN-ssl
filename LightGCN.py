...
class LightGCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        self.norm_adj_aug = data_config['norm_adj_aug']
        self.norm_adj_aug_2 = data_config['norm_adj_aug_2']
        '''
        *********************************************************
        ...
        *********************************************************
        ...
        if self.alg_type in ['lightgcn']:
            self.ua_embeddings, self.ia_embeddings, self.ua_aug_embeddings, self.ia_aug_embeddings, self.ua_aug_embeddings_2, self.ia_aug_embeddings_2 = self._create_ssl_lightgcn_embed()
        ...
        *********************************************************
        ...
        self.ssl_loss = self.calc_ssl_loss_augmented()
    ...
    # def _data_augmentation(self,embeddings):
    #     mask = np.random.choice([0, 1], size=embeddings.shape[0], p=[0.2, 0.8])
    #     mask = tf.convert_to_tensor(mask, dtype=tf.float32)
    #     augmented_embeddings = embeddings * tf.expand_dims(mask, axis=1)
    #     return augmented_embeddings
    
    def calc_ssl_loss_augmented(self):

    # User-side contrastive loss
    # Extract original and augmented user embeddings
        u_g_embeddings_aug = tf.nn.embedding_lookup(self.ua_aug_embeddings, self.users)
        u_g_embeddings_aug_2 = tf.nn.embedding_lookup(self.ua_aug_embeddings_2, self.users)
        normalize_user_emb_aug = tf.nn.l2_normalize(u_g_embeddings_aug, 1)
        normalize_user_emb_aug_2 = tf.nn.l2_normalize(u_g_embeddings_aug_2, 1)
        normalize_all_user_emb_aug_2 = tf.nn.l2_normalize(self.ua_aug_embeddings_2, 1)

        # Positive score (original vs augmented)
        pos_score_user = tf.reduce_sum(tf.multiply(normalize_user_emb_aug, normalize_user_emb_aug_2), axis=1)
        ttl_score_user = tf.matmul(normalize_user_emb_aug, normalize_all_user_emb_aug_2, transpose_a=False, transpose_b=True)

        # Apply temperature scaling and softmax
        pos_score_user = tf.exp(pos_score_user / 0.2)
        ttl_score_user = tf.reduce_sum(tf.exp(ttl_score_user / 0.2), axis=1)

        # Contrastive loss for users
        ssl_loss_user = -tf.reduce_sum(tf.log(pos_score_user / ttl_score_user))

        # Item-side contrastive loss
        # Extract original and augmented item embeddings
        i_g_embeddings_aug = tf.nn.embedding_lookup(self.ia_aug_embeddings, self.pos_items)
        i_g_embeddings_aug_2 = tf.nn.embedding_lookup(self.ia_aug_embeddings_2, self.pos_items)
        normalize_item_emb_aug = tf.nn.l2_normalize(i_g_embeddings_aug, 1)
        normalize_item_emb_aug_2 = tf.nn.l2_normalize(i_g_embeddings_aug_2, 1)
        normalize_all_item_emb_aug_2 = tf.nn.l2_normalize(self.ia_aug_embeddings_2, 1)

        # Positive score (original vs augmented)
        pos_score_item = tf.reduce_sum(tf.multiply(normalize_item_emb_aug, normalize_item_emb_aug_2), axis=1)
        ttl_score_item = tf.matmul(normalize_item_emb_aug, normalize_all_item_emb_aug_2, transpose_a=False, transpose_b=True)

        # Apply temperature scaling and softmax
        pos_score_item = tf.exp(pos_score_item / 0.2)
        ttl_score_item = tf.reduce_sum(tf.exp(ttl_score_item / 0.2), axis=1)

        # Contrastive loss for items
        ssl_loss_item = -tf.reduce_sum(tf.log(pos_score_item / ttl_score_item))

        # Combine the user and item contrastive losses
        ssl_loss = 0.01 * (ssl_loss_user + ssl_loss_item)

        return ssl_loss



    # def _contrastive_loss(self, emb1, emb2, tau=0.5):
    #     emb1 = tf.nn.l2_normalize(emb1, axis=1)
    #     emb2 = tf.nn.l2_normalize(emb2, axis=1)
    #     sim_matrix = tf.matmul(emb1, emb2, transpose_b=True) / tau
    #     sim_matrix = tf.exp(sim_matrix)
        
    #     pos_sim = tf.linalg.diag_part(sim_matrix)
    #     loss = -tf.reduce_mean(tf.math.log(pos_sim / (tf.reduce_sum(sim_matrix, axis=1) - pos_sim)))
    #     return loss
    
    def _create_ssl_lightgcn_embed(self):
    # Step 1: Generate embeddings based on the original adjacency matrix
        ...
        # Propagate embeddings through layers for the original adjacency matrix
        ...
        # Split the embeddings into user and item embeddings for original adjacency matrix
        u_g_embeddings, i_g_embeddings = tf.split(all_embeddings, [self.n_users, self.n_items], 0)

        # Step 2: Generate embeddings based on the augmented adjacency matrix (Node Dropout)
        A_fold_hat_aug = self._split_A_hat(self.norm_adj_aug)  # Split augmented adj matrix for message passing
        
        # Initialize embeddings for augmented adjacency matrix
        ego_embeddings_aug = tf.concat([self.weights['user_embedding'], self.weights['item_embedding']], axis=0)
        all_embeddings_aug = [ego_embeddings_aug]
        
        # Propagate embeddings through layers for the augmented adjacency matrix
        for k in range(0, self.n_layers):
            temp_embed_aug = []
            for f in range(self.n_fold):
                temp_embed_aug.append(tf.sparse_tensor_dense_matmul(A_fold_hat_aug[f], ego_embeddings_aug))
            side_embeddings_aug = tf.concat(temp_embed_aug, 0)
            ego_embeddings_aug = side_embeddings_aug
            all_embeddings_aug += [ego_embeddings_aug]
        
        all_embeddings_aug = tf.stack(all_embeddings_aug, 1)
        all_embeddings_aug = tf.reduce_mean(all_embeddings_aug, axis=1, keepdims=False)
        
        # Split the embeddings into user and item embeddings for augmented adjacency matrix
        u_g_embeddings_aug, i_g_embeddings_aug = tf.split(all_embeddings_aug, [self.n_users, self.n_items], 0)

        A_fold_hat_aug_2 = self._split_A_hat(self.norm_adj_aug_2)  # Split augmented adj matrix for message passing
        
        # Initialize embeddings for augmented adjacency matrix
        ego_embeddings_aug_2 = tf.concat([self.weights['user_embedding'], self.weights['item_embedding']], axis=0)
        all_embeddings_aug_2 = [ego_embeddings_aug_2]
        
        # Propagate embeddings through layers for the augmented adjacency matrix
        for k in range(0, self.n_layers):
            temp_embed_aug_2 = []
            for f in range(self.n_fold):
                temp_embed_aug_2.append(tf.sparse_tensor_dense_matmul(A_fold_hat_aug_2[f], ego_embeddings_aug_2))
            side_embeddings_aug_2 = tf.concat(temp_embed_aug_2, 0)
            ego_embeddings_aug_2 = side_embeddings_aug_2
            all_embeddings_aug_2 += [ego_embeddings_aug_2]
        
        all_embeddings_aug_2 = tf.stack(all_embeddings_aug_2, 1)
        all_embeddings_aug_2 = tf.reduce_mean(all_embeddings_aug_2, axis=1, keepdims=False)
        
        # Split the embeddings into user and item embeddings for augmented adjacency matrix
        u_g_embeddings_aug_2, i_g_embeddings_aug_2 = tf.split(all_embeddings_aug_2, [self.n_users, self.n_items], 0)

        # Return both sets of embeddings (original and augmented)
        return u_g_embeddings, i_g_embeddings, u_g_embeddings_aug, i_g_embeddings_aug, u_g_embeddings_aug_2, i_g_embeddings_aug_2
    ...
    def _create_lightgcn_embed(self):
        ...
        # u_g_aug_embeddings = self._data_augmentation(u_g_embeddings)
        # i_g_aug_embeddings = self._data_augmentation(i_g_embeddings)
    ...
    def create_bpr_loss(self, users, pos_items, neg_items):
        ...
        # u_g_embeddings, i_g_embeddings, u_g_aug_embeddings, i_g_aug_embeddings = self._create_ssl_lightgcn_embed()
        # contrastive_loss_u = self._contrastive_loss(u_g_embeddings, u_g_aug_embeddings)
        # contrastive_loss_i = self._contrastive_loss(i_g_embeddings, i_g_aug_embeddings)
        # contrastive_loss = contrastive_loss_u + contrastive_loss_i
...
if __name__ == '__main__':
    ...
    config['norm_adj'] = data_generator.create_adj_mat(aug_type = 'None')
    config['norm_adj_aug'] = data_generator.create_adj_mat(aug_type = 'Node')
    config['norm_adj_aug_2'] = data_generator.create_adj_mat(aug_type = 'Node')
    """
    *********************************************************
    ...
    *********************************************************
    ...
    users_to_test = list(data_generator.test_set.keys())
    best_test_prediction = test(sess, model, users_to_test, drop_flag=True)['prediction']  # Modify according to your output structure

    # Save best iteration predictions to a file
    result_save_path = '%soutput/%s/%s_result_ssl.txt' % (args.proj_path, args.dataset, model.model_type)
    with open(result_save_path, 'w') as f:
        for user_id, prediction in enumerate(best_test_prediction):
            f.write(f"{user_id} {prediction}\n")

    print(f"Best iteration predictions saved to {result_save_path}")