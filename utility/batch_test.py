...
def test(sess, model, users_to_test, drop_flag=False, train_set_flag=0):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        all_prediction.append(rate_batch)  # Collect the predictions
        test_items = []
        if train_set_flag == 0:
    ...
    all_prediction = np.concatenate(all_prediction, axis=0)  # Combine all prediction batches
    result['prediction'] = all_prediction  # Add predictions to result
    return result
