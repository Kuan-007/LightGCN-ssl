...
class Data(object):
        ...
        with open(train_file) as f:
            for l in f.readlines():
                if len(l) > 0:
                    ...
                    # 将用户和物品分别存储到 self.training_user 和 self.training_item
                    self.training_user.extend([uid] * len(items))
    ...
    def create_adj_mat(self, aug_type):
        n_nodes = self.n_users + self.n_items
        if aug_type == 'Node':
            drop_user_idx = np.random.choice(self.n_users, size=int(self.n_users * 0.1), replace=False)
            drop_item_idx = np.random.choice(self.n_items, size=int(self.n_items * 0.1), replace=False)
